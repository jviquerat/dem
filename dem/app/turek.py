...
class turek(base_app):
    ...
    def __init__(self,
        ...
        self.rmv_freq = 200 # check particles removal every 10 iterations
        self.add_freq = 500 # add new particles every 10 iterations
    ...
    def forces(self):
        ...
        if (self.it%self.add_freq == 0):
            ...
            for i in range(n):
                x[i,0] = 0.5*dx + i*dx + self.p.radius*random.random()
                x[i,1] = self.d.y_max - 0.5

            self.p.add(n, m, r, x, v, c)
            force_nearest = True
