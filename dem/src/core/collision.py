...
def hertz(dx, r1, r2, m1, m2, v1, v2, n, t,
    ...
    fne[0] = np.power(dx,1.5)*k_n*n[0]
    fne[1] = np.power(dx,1.5)*k_n*n[1]

    # normal damping force
    fnd[0] =-np.power(dx,0.25)*nu_n*vn*n[0]
    fnd[1] =-np.power(dx,0.25)*nu_n*vn*n[1]

    # tangential elastic force
    #p.f[i,:] -= pow(dx,0.5)*k_t*vt*0.00001*t[:]

    # tangential damping force
    ftd[0] =-np.power(dx,0.25)*nu_t*vt*t[0]
    ftd[1] =-np.power(dx,0.25)*nu_t*vt*t[1]
